  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h2 className="font-display text-2xl font-bold text-foreground">Teaching Approach</h2>
        <div className="flex gap-2">
          {orderChanged && <Button size="sm" onClick={saveOrder}><Save size={14} className="mr-1" />Save Order</Button>}
          <Button size="sm" onClick={() => { setAdding(true); setEditing(null); setForm(empty); }}><Plus size={14} className="mr-1" />Add Approach</Button>
        </div>
      </div>
      ...
      {(adding || editing) && (
        <div className="glass-card p-6 space-y-4">
          <div className="flex justify-between items-center">
            <h3 className="font-semibold text-foreground">{editing ? "Edit" : "Add"} Approach</h3>
            <Button variant="ghost" size="sm" onClick={() => { setAdding(false); setEditing(null); }}><X size={16} /></Button>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div><Label>Title (EN)</Label><Input value={form.title_en} onChange={e => setForm({ ...form, title_en: e.target.value })} className="mt-1" /></div>
            <div><Label>Title (BN)</Label><Input value={form.title_bn} onChange={e => setForm({ ...form, title_bn: e.target.value })} className="mt-1" /></div>
