...
import { Eye, Download, Video, Clock, Trash2, CalendarClock, TrendingUp } from "lucide-react";
...
// ─── Exported individual widgets ───

export const MostViewedPostsWidget = ({ data }: { data: ViewItem[] }) => (
  <Card>
    ...
    <CardContent>
      <MiniList items={data.map(viewRow)} emptyMsg="No view data yet" />
    </CardContent>
  </Card>
);

export const MostViewedMaterialsWidget = ({ data }: { data: ViewItem[] }) => (
  <Card>
    <CardHeader className="pb-2">
      <CardTitle className="text-sm flex items-center gap-2">
...
export const MostWatchedVideosWidget = ({ data }: { data: ViewItem[] }) => (
  <Card>
    <CardHeader className="pb-2">
      <CardTitle className="text-sm flex items-center gap-2">
...
export const RecentUploadsWidget = ({ data }: { data: RecentItem[] }) => (
  <Card>
    <CardHeader className="pb-2">
      <CardTitle className="text-sm flex items-center gap-2">
...
export const ScheduledContentWidget = ({ data }: { data: ScheduledItem[] }) => (
  <Card>
    <CardHeader className="pb-2">
      <CardTitle className="text-sm flex items-center gap-2">
...
export const TrashSummaryWidget = ({ data }: { data: TrashCount[] }) => (
  <Card>
    <CardHeader className="pb-2">
      <CardTitle className="text-sm flex items-center gap-2">
...
export interface InsightsData {
  topPosts: ViewItem[];
  topMaterials: ViewItem[];
  topVideos: ViewItem[];
  recent: RecentItem[];
  scheduled: ScheduledItem[];
  trashCounts: TrashCount[];
  loading: boolean;
}
...
export const useInsightsData = (): InsightsData => {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      const blogViews = views.filter((v: any) => v.content_type === "blog_post").slice(0, 5);
      const matViews = views.filter((v: any) => v.content_type === "study_material").slice(0, 5);
      const vidViews = views.filter((v: any) => v.content_type === "video").slice(0, 5);

      const resolveNames = async (items: any[], table: string): Promise<ViewItem[]> => {
        if (items.length === 0) return [];
        const ids = items.map((i: any) => i.content_id);
        const { data } = await (supabase as any).from(table).select("id, title").in("id", ids);
        const titleMap: Record<string, string> = {};
        data?.forEach((d: any) => { titleMap[d.id] = d.title; });
        return items.map((i: any) => ({ content_id: i.content_id, view_count: i.view_count, title: titleMap[i.content_id] || "Unknown" }));
      };
  ...
  return { topPosts, topMaterials, topVideos, recent, scheduled, trashCounts, loading };
};
...
const AdminInsightsWidgets = () => {
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
      <MostViewedPostsWidget data={data.topPosts} />
      <MostViewedMaterialsWidget data={data.topMaterials} />
      <MostWatchedVideosWidget data={data.topVideos} />
      <RecentUploadsWidget data={data.recent} />
      <ScheduledContentWidget data={data.scheduled} />
      <TrashSummaryWidget data={data.trashCounts} />
    </div>
  );
};
