                ...
                <SortableRow key={item.id} id={item.id}>
                  <div className="glass-card p-2.5 sm:p-4 flex items-center justify-between gap-2 sm:gap-3">
                    <div className="flex-1 min-w-0 overflow-hidden">
                      <p className="font-medium text-foreground truncate text-sm sm:text-base">{item.degree_title_en || "Untitled"}</p>
                      <p className="text-xs sm:text-sm text-muted-foreground truncate">{item.institution_en} · {item.year}</p>
                    </div>
