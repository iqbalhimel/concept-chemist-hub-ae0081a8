    ...
    <section id="testimonials" className="section-padding section-gradient">
      <div className="container mx-auto">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-5 max-w-6xl mx-auto">
          {items.map((item, i) => {
            const text = lang === "bn" && item.testimonial_text_bn ? item.testimonial_text_bn : item.testimonial_text_en || item.testimonial_text_bn;
            return (
              <motion.div key={item.id} variants={fadeUp} initial="hidden" whileInView="visible" viewport={vp} transition={{ duration: 0.5, delay: i * 0.08 }} className="glass-card-hover p-6 flex flex-col gap-4">
